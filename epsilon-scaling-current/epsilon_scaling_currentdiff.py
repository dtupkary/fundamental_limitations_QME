...
epsilonindex=0
N=3
diff=np.zeros((10,N-2),dtype=np.float64) #stores the successive differences in currents.
...
######################## NOw we do universal lindblad shit. #######################################
    ...
    print('max - min site current for RQE :',max(vec_red)-min(vec_red))
    print('max - min site current for RRQE :',max(vec_realred)-min(vec_realred))
    print('max - min site current for LLE :',max(vec_lindblad)-min(vec_lindblad))
    print('max - min site current for ULE :', max(vec_uni)-min(vec_uni))
