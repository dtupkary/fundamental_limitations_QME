...
data=np.loadtxt("./N=3,10delta=10,current_withuleboundary.txt")
...
curr_universal=[y for _, y in sorted(zip(data[:,0],data[:,4]))  ]
gvals=sorted(data[:,0])
...
ax11.plot(gvals,curr_universal,'b.-',label='ULE(Boundary)',markersize=size1,linewidth=size2)
...
ax11.text(0.025,4*0.0011,string,fontsize=10,bbox=dict(facecolor='none', edgecolor='black', pad=2.0))
...
fig.tight_layout()
plt.savefig('currentplot_withboundary',dpi=500,bbox_inches='tight')
