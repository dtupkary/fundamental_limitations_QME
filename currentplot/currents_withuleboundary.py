# -*- coding: utf-8 -*-
"""
Created on Wed Feb 24 10:20:40 2021
...
Computes Current vs g. ULE current is bounary. Rest currents are site. We use 4* earlier current definition. To be run on computer
@author: Devashish
"""
import os.path
import sys
...
gvals=np.linspace(0.01,0.25,15)
...
for g in gvals:
    ...
    current_op=4.0j*g*(create_sm(N,1).dag()*create_sm(N,2)-create_sm(N,1)*create_sm(N,2).dag())
    #universal_current.append(expect(current_op,ss_universal))
    redfield_current.append(expect(current_op,ss_redfield))
    realredfield_current.append(expect(current_op,ss_realredfield))
    locallindblad_current.append(expect(current_op,ss_lindblad))
    ...
    for pos in range(1,N):
        current_op=1.0j*g*(create_sm(N,pos).dag()*create_sm(N,pos+1)-create_sm(N,pos)*create_sm(N,pos+1).dag())
    ...
    leftcurrent=ule_left_ls+ule_left_lindblad
    rightcurrent=ule_right_ls+ule_right_lindblad
    
    if (np.abs(leftcurrent+rightcurrent )>=10e-10):
        print('PROBLEM HERE. Boundary currents dont match')
    universal_current.append(rightcurrent.real)
...
string="N={},w0max={},w0min={},tb={},epsilon={},gamma1={},gamma2={}, delta={}, \n mu1={},mu2={},beta1={},beta2={} \n GVALS || REDFIELD || REALREDLFIELD || LINDBLAD || UNIVERSAL|| TRACE DIST (red, ule)".format(N,w0max,w0min,tb,epsilon,gamma1,gamma2,delta,mu1,mu2,beta1,beta2)

data=np.column_stack((gvals,redfield_current,realredfield_current,locallindblad_current,universal_current))
header_string=string+"\n GVALS || REDFIELD || REALREDFIELD || LOCALLINDBLAD || ULE BOUNDARY "
np.savetxt('N={},10delta={},current_withuleboundary.txt'.format(N,10*delta),data,header=header_string)
...
"""    
qsave(ss_redfield,'ss_redfield,N={},delta={},gindex={},current'.format(int(N),int(10*delta),int(gindex)))
qsave(ss_realredfield,'ss_realredfield,N={},delta={},gindex={},current'.format(int(N),int(10*delta),int(gindex)))
qsave(ss_lindblad,'ss_lindblad,N={},delta={},gindex={},current'.format(int(N),int(10*delta),int(gindex)))
qsave(ss_universal,'ss_universal,delta={},gindex={},current'.format(int(N),int(10*delta),int(gindex)))
