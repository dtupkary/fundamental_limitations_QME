...
computes thermalization error between theory and redfield vs g. (saves data in file. To be run on computer,
 does computation serially )
Lamb shift IS included in this!
@author: Devashish
"""
...
for g in gvals:
    ...
    print('RealRedfield Liouvillian constructed, Computing stead-state ...')
    ...
    print('Lambshift for full completed, Liouvillian constructed, computing steady state ...')
    ...
    #first save data
    redfield_dist.append(tracedist(ss_redfield,ss_theory))
    realredfield_dist.append(tracedist(ss_realredfield,ss_theory))
    locallindblad_dist.append(tracedist(ss_lindblad,ss_theory))
    universal_dist.append(tracedist(ss_universal,ss_theory))



###### store the data

string="N={},w0max={},w0min={} tb={},epsilon={},gamma1={},gamma2={}\n delta={}, mu1={},mu2={},beta1={},beta2={} ".format(N,w0max,w0min,tb,epsilon,gamma1,gamma2,delta,mu1,mu2,beta1,beta2)

data=np.column_stack((gvals,redfield_dist,realredfield_dist,locallindblad_dist,universal_dist))
header=string+"\n GVALS || REDFIELD || REALREDFIELD || LOCALLINDBLAD  || UNIVERSAL LINDBLAD"
np.savetxt("N={},10delta={}_thermalization.txt".format(N,10*delta),data,header=header)

############# We alsow want to store the matrices. Which means, we need a good way of writing their names #######
    ## we just multiple g by 10 and save that (as int) ##
...
"""
qsave(ss_redfield,'ss_redfield,N={},delta={},gindex={},thermalization'.format(int(N),int(10*delta),int(gindex)))
qsave(ss_realredfield,'ss_realredfield,N={},delta={},gindex={},thermalization'.format(int(N),int(10*delta),int(gindex)))
qsave(ss_lindblad,'ss_lindblad,N={},delta={},gindex={},thermalization'.format(int(N),int(10*delta),int(gindex)))
qsave(ss_universal,'ss_universal,delta={},gindex={},thermalization'.format(int(N),int(10*delta),int(gindex)))
qsave(ss_theory,'ss_theory,N={},delta={},gindex={},thermalization'.format(int(N),int(10*delta),int(gindex)))
