...
Computes and plots the site current in all 4 approaches
For RQE, RRQE and ULE we plot only one site current.
For ULE we plot 2 site currents for the n=3 case.

This code is only meant to be used for N=3. We will compute the lambshift first so that
it is not computed for every epsilon.
@author: Devashish
"""
...
redfield_current=np.zeros((9))
locallindblad_current=np.zeros((9))
universal_current_site1=np.zeros((9))
universal_current_site2=np.zeros((9))
universal_boundary=np.zeros((9))

## first we do cals for epsilon=1
print('Doing lamb shift computation for epsilon=1')
epsilon=1
...
######################## NOw we do universal lindblad shit. #######################################
    ...
    L_redfield=L
    L_universal=liouvillian(H_S+ls,Cops_universal)
    ...
    print('Redfield currents are ',vec_red)
    print('locallindblad currents are ',vec_lindblad)
    print('ule currents are ',vec_uni)
    
    redfield_current[epsilonindex]=vec_red[0]
    locallindblad_current[epsilonindex]=vec_lindblad[0]
    universal_current_site1[epsilonindex]=vec_uni[0]
    universal_current_site2[epsilonindex]=vec_uni[1]
    
    
    ## now we compute ULe boundary current
    Mz=create_magnetization(N)
    ule_left_ls=(1.0j*ss_universal*commutator( epsilon*epsilon*ls_left , Mz  )).tr()
    ule_right_ls=(1.0j*ss_universal*commutator( epsilon*epsilon*ls_right, Mz  )).tr()
    
    ule_left_lindblad=(ss_universal*(Cops_1.dag()*Mz*Cops_1-0.5*Mz*Cops_1.dag()*Cops_1-0.5*Cops_1.dag()*Cops_1*Mz + Cops_2.dag()*Mz*Cops_2-0.5*Mz*Cops_2.dag()*Cops_2-0.5*Cops_2.dag()*Cops_2*Mz )).tr()
    ule_right_lindblad= (ss_universal*(Cops_3.dag()*Mz*Cops_3-0.5*Mz*Cops_3.dag()*Cops_3-0.5*Cops_3.dag()*Cops_3*Mz + Cops_4.dag()*Mz*Cops_4-0.5*Mz*Cops_4.dag()*Cops_4-0.5*Cops_4.dag()*Cops_4*Mz )).tr()
    
    print('ULe boudary currents are ',[ule_left_ls+ule_left_lindblad,ule_right_ls+ule_right_lindblad] )
    
    
    universal_boundary[epsilonindex]=np.max(np.abs([ule_left_ls+ule_left_lindblad,ule_right_ls+ule_right_lindblad]))
...
string="N={},w01={},w02={},w03={},tb={},g={},gamma1={},gamma2={}, delta={}, \n mu1={},mu2={},beta1={},beta2={}".format(N,w0list[0],w0list[1],w0list[2],tb,g,gamma1,gamma2,delta,mu1,mu2,beta1,beta2)
data=np.column_stack((epsilonvals,redfield_current,locallindblad_current,universal_current_site1,universal_current_site2,universal_boundary))
np.savetxt('epsilonscaling_currents_withboundary.txt',data,header=string+'Epsilon || REDFIELD ||  LLE || ULE SITE 1 || ULE SITE 2 || ULE BOUNDARY')
...
# ax1.set_ylabel(r"$I_1-I_2$")
# ax1.set_title(string,fontsize=8)

# for i in range(N-2):
#     plt.loglog(epsilonvals,diff[:,i],label="{}-{}. Slope={}".format(i+1,i+2,(np.log(diff[7,i])-np.log(diff[0,i]))/(np.log(epsilonvals[7])-np.log(epsilonvals[0]))))
